    ...
    GetMetricsInfo: function () {
      ...
      return {
        ...
        webAssemblyStartupTime:
          Module.HEAPU32[webAssemblyStartupTimePtr >> 2] -
          (Module.webAssemblyTimeStart || 0),
      ...
      try {
        ...
        if (
          !WebAssembly.validate(
            new Uint8Array([
              ...
              1, 0, 1, 10, 13, 1, 11, 0, 65, 0, 65, 0, 65, 1, 252, 11, 0, 11,
            ]),
          )
        )
          return "bulk-memory";
        if (
          !WebAssembly.validate(
            new Uint8Array([
              ...
              11, 1, 9, 1, 1, 125, 32, 0, 252, 0, 26, 11,
            ]),
          )
        )
        ...
        if (
          !WebAssembly.validate(
            new Uint8Array([
              0, 97, 115, 109, 1, 0, 0, 0, 1, 4, 1, 96, 0, 0, 3, 2, 1, 0, 10,
              10, 1, 8, 0, 6, 64, 1, 25, 1, 11, 11,
            ]),
          )
        )
          return "wasm-exceptions";
        return true;
      } catch (e) {
        return "Exception: " + e;
      }
      ...
      hasUnityMarker: function (data) {
        ...
        return (
          String.fromCharCode.apply(
            ...
            data.subarray(
              commentOffset,
              commentOffset + expectedComment.length,
            ),
          ) == expectedComment
        );
      },
  ...
  function downloadBinary(urlId) {
    ...
    return request
      .then(function (response) {
        ...
        var compression;
        if (decompressors.gzip.hasUnityMarker(response.parsedBody))
          compression = ["gzip", "gzip"];
        if (decompressors.br.hasUnityMarker(response.parsedBody))
        ...
        if (compression) {
          ...
          var browserDidDecompress =
            compressedLength > 0 &&
            response.parsedBody.length != compressedLength;
          var browserDidNotDecompress =
            compressedLength > 0 &&
            response.parsedBody.length == compressedLength;

          if (encoding != compression[1])
            showBanner(
    ...
    Module.preRun.push(function () {
      ...
      dataPromise.then(function (data) {
        ...
        function fail(reason) {
          if (decompressors.gzip.hasUnityMarker(data))
            throw (
              reason +
          ...
          if (decompressors.br.hasUnityMarker(data))
            throw (
              reason +
